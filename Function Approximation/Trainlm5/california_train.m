...
for num_neurons_second = 10:10:50
    for num_neurons = 10:10:50
        [net] = newff(minmax(P_train),[num_neurons num_neurons_second num_neurons_third 1],{'tansig','tansig','tansig'},'trainlm');
        ...
        Missclassification_rate((num_neurons-10)/5+1,1) = sqrt(mean((T_test - est).^2));
        mse_performance((num_neurons-10)/5+1,1) = perform(net,T_test,est);
        
        %plot and save the performance figure
        name = ['Trainlm5\performance_',num2str(num_neurons_second),'_',num2str(num_hidden_layers),'_',num2str(num_neurons)];
        h=figure;
        plotperform(tr);
        saveas(h,name,'jpg');
        
        %plot and save trainstate
        name = ['Trainlm5\plottrainstate',num2str(num_neurons_second),'_',num2str(num_hidden_layers),'_',num2str(num_neurons)];
        h=figure;
        plottrainstate(tr);
        saveas(h,name,'jpg');
        
        %plot and save regression
        name = ['Trainlm5\plotregression',num2str(num_neurons_second),'_',num2str(num_hidden_layers),'_',num2str(num_neurons)];
        h=figure;
        ...
        name = ['Trainlm5\plothistogram',num2str(num_neurons_second),'_',num2str(num_hidden_layers),'_',num2str(num_neurons)];
        h=figure;
        ploterrhist(errors);
        saveas(h,name,'jpg');
    ...
    name = ['Trainlm5\misclassification_rate',num2str(num_neurons_second)];
    h=figure;
    ...
    name = ['Trainlm5\mseperformance_rate',num2str(num_neurons_second)];
    saveas(h,name,'jpg');
end
