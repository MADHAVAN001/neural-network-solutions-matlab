...
for num_neurons = 10:10:50
    for num_neurons_second = 10:10:50
        ...
        Missclassification_rate((num_neurons_second-10)/10 + 1,1) = sqrt(mean((T_test - est).^2));
        mse_performance((num_neurons_second-10)/10 + 1,1) = perform(net,T_test,est);
        
        %plot and save the performance figure
        name = ['Trainlm_2_layers\performance_maxfail',num2str(num_neurons),'_',num2str(num_neurons_second)];
        h=figure;
        plotperform(tr);
        saveas(h,name,'jpg');
        
        %plot and save trainstate
        name = ['Trainlm_Varylayers\plottrainstate',num2str(num_neurons),'_',num2str(num_neurons_second)];
        h=figure;
        plottrainstate(tr);
        saveas(h,name,'jpg');
        
        %plot and save regression
        name = ['Trainlm_Varylayers\plotregression',num2str(num_neurons),'_',num2str(num_neurons_second)];
        h=figure;
        ...
        name = ['Trainlm_Varylayers\plothistogram',num2str(num_neurons),'_',num2str(num_neurons_second)];
        h=figure;
        ploterrhist(errors);
        saveas(h,name,'jpg');
    ...
    name = ['Trainlm_Varylayers\misclassification_rate',num2str(num_neurons)];
    h=figure;
    ...
    name = ['Trainlm_Varylayers\mseperformance_rate',num2str(num_neurons)];
    saveas(h,name,'jpg');
    %end
